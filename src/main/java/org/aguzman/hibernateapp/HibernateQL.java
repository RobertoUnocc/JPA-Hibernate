...
public class HibernateQL {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("===CONSULTA POR CASOS PERSONALIZADOS ===");
        ...
        System.out.println("===CONSULTA POR CASOS PERSONALIZADOS LISTA ===");
        List<Object[]> registros = em.createQuery("select c.id,c.nombre,c.apellido from Cliente c", Object[].class)
                .getResultList();

        registros.forEach(reg ->{
                Long idCli= (Long) objetoCliente[0];
                String nombreCli= (String) objetoCliente[1];
                String apellidoCli= (String) objetoCliente[2];
        ...
        */
        System.out.println(" === CONSULTA POR NOMBRE DE CLIENTE ===");
        List<String> nombreClientes=  em.createQuery("select c.nombre from Cliente c", String.class).getResultList();
        nombreClientes.forEach(c-> System.out.println(c));

        System.out.println(" === CONSULTA POR NOMBRES UNICOS DE CLIENTE ===");
        nombreClientes=  em.createQuery("select distinct(c.nombre) from Cliente c", String.class).getResultList();
        nombreClientes.forEach(c-> System.out.println(c));

        System.out.println(" === CONSULTA POR CANTIDAD DE FORMAS DE PAGOS UNICOS DE CLIENTE ===");
       Long totalFormaPago =  em.createQuery("select count(distinct(c.formaPago)) from Cliente c", Long.class).getSingleResult();        nombreClientes.forEach(c-> System.out.println(c));
        System.out.println(totalFormaPago);

        System.out.println("=== CONSULTA NOMBRE,APELLIDO CONCATENADO ===");
        nombreClientes =em.createQuery("select upper(concat(c.nombre,' ',c.apellido)) as nombreCompleto from Cliente c", String.class).getResultList();
        nombreClientes.forEach(c-> System.out.println(c));


        System.out.println("==CONSULTAR BUSCAR POR NOMBRE==");
        String param="s";
